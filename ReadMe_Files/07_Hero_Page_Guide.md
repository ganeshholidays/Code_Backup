...
## Mobile Header Behavior per Hero Mode

On mobile, the navbar behavior changes based on the hero mode:

| Mode | Mobile Header | Mobile Hero starts |
|------|-------------- |-------------------|
| **Festival** | Solid dark navy (always) | Below header (padding-top: 70px) |
| **Video** | Solid dark navy (always) | Below header (padding-top: 70px) |
| **Hero-default** | Transparent → solid on scroll | Behind transparent header (full screen) |

On desktop, the header is **always transparent at top → solid on scroll** for all modes.

### How it works:
- When festival or video is detected, JS adds `hero-has-media` class to `<body>`
- CSS rule `body.hero-has-media .navbar` (mobile only) makes navbar solid
- Festival and video also get `padding-top: 70px` on mobile so content starts below the solid header
