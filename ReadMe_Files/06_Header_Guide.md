...
## Mobile Navbar for Festival & Video Modes

When festival.jpg or hero-video.mp4 is active, the navbar is **always solid dark navy on mobile** (not transparent). This prevents the transparent header from hiding the festival/video content on mobile.

### How it works:
- JS adds `hero-has-media` class to `<body>` when festival or video is detected
- CSS targets `body.hero-has-media .navbar` inside a `@media (max-width: 768px)` rule

### Current behavior:

| Mode | Mobile | Laptop |
|------|--------|--------|
| Festival | Solid dark navy always | Transparent → solid on scroll |
| Video | Solid dark navy always | Transparent → solid on scroll |
| Hero-default | Transparent → solid on scroll | Transparent → solid on scroll |

### CSS (in `style.css`):
```css
@media (max-width: 768px) {
    body.hero-has-media .navbar {
...
### JS (in `main.js`):
```javascript
// Added when festival detected:
document.body.classList.add('hero-has-media');

// Added when video detected:
document.body.classList.add('hero-has-media');

// Removed if video autoplay fails:
